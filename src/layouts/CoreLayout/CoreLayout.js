...
export const _Examples = ({ examples, selected, showExample }) => (
  ...
  <div className="sidebar-dos">
    <Menu title="Commands">
      <Menu.Item
        icon="save"
        shortcut="Ctrl+S"
        onClick={saveSnippet.bind(null, editors["es6"].code)}
      >
        save code
      </Menu.Item>
      <Menu.Item
      ...
      <Menu.Item icon="refresh" shortcut="Ctrl+B" onClick={transformCode}>
        transform
      </Menu.Item>
    </Menu>
    ...
    {selectedCompiler == "Babel (6)" ? (
      <Menu title="Presets" helpHref="http://babeljs.io/docs/plugins/#presets">
        {Object.keys(babelPresets).map((key) => (
          <Menu.Item
            key={key}
            icon={babelPresets[key].checked ? "toggle on" : "toggle off"}
      ...
      <div className="core-layout__viewport">
        ...
        <nav>
          <div className="sidebar-uno">
            ...
            <a
              className="sidebar-uno__bottom"
              href="//github.com/matthisk/es6console"
            >
              <div className="button-round">
                <Icon name="github" />
              </div>
          ...
          {this.state.selected == "toolbar" ? <Toolbar /> : null}
          {this.state.selected == "examples" ? <Examples /> : null}
        </nav>
